...
public class SignInputGUI implements Listener {
    ...
    private void runOnPlayer(Runnable task, long delayTicks) {
        codes.castled.gpexpansion.scheduler.SchedulerAdapter.runLaterEntity(plugin, player, task, delayTicks);
    }
    
    private void runAtSignLocation(Runnable task) {
        if (signLocation != null) {
            codes.castled.gpexpansion.scheduler.SchedulerAdapter.runAtLocation(plugin, signLocation, task);
        }
    }
    
    private void runAtSignLocationDelayed(Runnable task, long delayTicks) {
        if (signLocation != null) {
            codes.castled.gpexpansion.scheduler.SchedulerAdapter.runAtLocationLater(plugin, signLocation, task, delayTicks);
        }
    }
